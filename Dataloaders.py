...
data = []
n_samples_of_each_class = 5000

prefix = 'Datasets1/'

for _ in range(n_samples_of_each_class):
    anchor_person = randrange(1, 10)
    ...
    positive = [base_path_org % (anchor_person, anchor_sign), base_path_org % (anchor_person, pos_sign), 0]
    negative = [base_path_org % (anchor_person, anchor_sign), base_path_forg % (anchor_person, neg_sign), 1]
...
with open('train_index.pkl', 'rb') as train_index_file:
	s1 = pickle.load(train_index_file)
